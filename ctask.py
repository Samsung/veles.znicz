...
class ctask(units.Unit):
    ...
    def test1(self):
        ...
        if 'NUMBER' not in self.param:
            print(" ERROR!!!! in taskhave not NUMBER")
            return -1
        if 'models_train' not in self.param:
            print(" ERROR!!!! in task have not models_train")
            return -2
        if 'data_set' not in self.param:
            print(" ERROR!!!! in task have not data_set")
            return -3
        if type(self.param['data_set'])!=dict:
            print(" ERROR!!!! in task data_set is not dict")
            return -4
        if type(self.param['models_train'])!=dict:
            print(" ERROR!!!! in task model_train is not dict")
            return -5
        _t=self.param['models_train']
        if 'model0' not in _t:
            print(" ERROR!!!! in task have not model_train")
            return -6
        if type(_t['model0'])!=dict:
            print(" ERROR!!!! in task  in model_train  model0 is not dict")
            return -7
        _tt=_t['model0']
        ...
        if type(_tt['data_model'])!=str:
            print(" ERROR!!!! in task  in model_train.model0.data_model is not str")
            return -9
        
        _t=self.param['data_set']
        if 'data' not in _t:
            print(" ERROR!!!! in task  data_set.data  not")
            return -10
        if type(_t['data'])!=str:
            print(" ERROR!!!! in task  in data_set.data is not str")
            return -11
        return 1
        ...
        try:
            ...
            if name in self._wf.__dict__:
                ...
                if not type(self.class_wf) is self.class_wf.__class__:
                    print(" ERROR!!!! in ",self.str_data_model, " object " ,name," is not class")
                    return -3
            else:
            ...
            try:
                ...
                if name in self._data_module.__dict__:
                    ...
                    if type(self.data_set['data_dict'])!=dict:
                        print(" ERROR!!!! in ",self.str_data, " object " ,name," is not dict")
                        return -3
                else:
    ...
    def run_model_WF(self):
        ...
        # global_alpha = 0.9, global_lambda = 0.0, threshold = 1.0,  надо добавить чтение параметров обучени из конфигов
        self.WF.run()
    ...
    def run(self):
        ...
        if(r<0):
            print(" ERROR!!!! in task",self.__getattribute__('NUMBER'))
            return 2
        ...
        print("                    RD ",r)
        if(r<0):
            print("ERROR!!!! not read config data")
            return 4
